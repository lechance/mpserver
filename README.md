...
错误处理：微信提交错误（非 0）→ 502；`code` 无效 → 400；用户近两小时未访问小程序（`61010`）→ 502 `用户近两小时未访问小程序，请重新进入小程序`；未配置 → 500 `服务未配置`；图片过大 → 413；类型/文件头不符 → 415。
...
- `uploads/` 定时清理：超过 24 小时未修改的图片会被自动删除（微信在提交后几分钟内即完成下载）。
