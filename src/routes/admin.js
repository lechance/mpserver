...
.fmt-chip{display:inline-flex;align-items:center;gap:3px;padding:3px 8px;border-radius:4px;font-size:12px;background:var(--pending-bg);color:var(--pending-text);border:1px solid var(--pending-border)}
.fmt-cards{display:flex;flex-direction:column;gap:8px}
.fmt-card{padding:10px 12px;border:1px solid var(--border);border-radius:6px;background:var(--bg-card)}
.fmt-kv{display:grid;grid-template-columns:auto 1fr;gap:4px 12px;font-size:13px;padding:10px 0}
.fmt-kv dt{color:var(--text-muted);font-weight:500;white-space:nowrap}
.fmt-kv dd{color:var(--text-primary);word-break:break-all;margin:0}
.fmt-note{font-size:12px;color:var(--text-muted);margin-top:6px}
.fmt-plain{font-size:13px;color:var(--text-primary);white-space:pre-wrap;word-break:break-all;line-height:1.6}
.fmt-empty{font-style:italic;color:var(--text-muted);font-size:13px}
.fmt-lbl{font-size:11px;color:var(--text-muted);text-transform:uppercase;margin-bottom:4px}
.fmt-section{margin-top:12px}
.fmt-section:first-child{margin-top:0}
.fmt-tool-name{color:var(--accent)}
</style>
...
<div class="modal-overlay" id="dataModal">
  <div class="data-modal">
    ...
    <div class="dm-body"><div id="dmContent" data-raw=""></div></div>
  </div>
</div>
...
function showDataModal(scope,dataType,content,updatedAt){document.getElementById('dmScope').textContent=scopeLabel(scope);document.getElementById('dmDataType').textContent=dataType;var parsed=parseData(content).parsed;var el=document.getElementById('dmContent');el.innerHTML=renderFormatted(scope,dataType,parsed);var raw=typeof content==='string'?content:JSON.stringify(content);el.setAttribute('data-raw',raw);document.getElementById('dmTime').textContent=fmtTime(updatedAt);dataModal.classList.add('show')}
function closeDataModal(){dataModal.classList.remove('show')}
function copyDataModalContent(){copyText(document.getElementById('dmContent').getAttribute('data-raw')||'')}
...
var TOOL_MAP={};TOOLS_CATALOG.forEach(function(c){c.tools.forEach(function(t){TOOL_MAP[t.id]=t})});
function toolName(id){var t=TOOL_MAP[id];return t?(t.icon||'')+' '+t.name:null}
function toolNameShort(id){var t=TOOL_MAP[id];return t?t.name:id}
function scopeLabel(s){if(s==='_global')return '\uD83C\uDF10 全局数据';if(s==='_profile')return '\uD83D\uDC64 用户资料';if(s==='_sync')return '\u2601\uFE0F 同步配置';var n=toolName(s);return n||s}
function parseData(data){if(data===null||data===undefined)return{parsed:null,isRaw:true};if(typeof data==='object')return{parsed:data,isRaw:false};if(typeof data==='string'){try{return{parsed:JSON.parse(data),isRaw:false}}catch(e){}}return{parsed:String(data),isRaw:true}}
function summarizeData(scope,dataType,parsed){if(parsed===null||parsed===undefined)return '\u7A7A';if(typeof parsed==='string')return parsed.length>60?parsed.slice(0,60)+'\u2026':parsed;if(Array.isArray(parsed)){if(!parsed.length)return '\u7A7A\u6570\u7EC4';if(scope==='_global'&&dataType==='favorites'){var names=parsed.map(function(id){return toolNameShort(id)});var s=names.slice(0,3).join('\u3001');return '\u6536\u85CF '+parsed.length+' \u4E2A\uFF1A'+s+(names.length>3?'\u2026':'')}if(scope==='_global'&&dataType==='search_history')return '\u641C\u7D22\u5386\u53F2 '+parsed.length+' \u6761\uFF1A'+parsed.slice(0,3).join('\u3001')+(parsed.length>3?'\u2026':'');if(scope==='_global'&&dataType==='records'){var first=parsed[0];return parsed.length+' \u6761\u4F7F\u7528\u8BB0\u5F55\u2022 \u6700\u8FD1\uFF1A'+(first&&first.toolName?first.toolName:first&&first.toolId?toolNameShort(first.toolId):'\u672A\u77E5')}return '\u5171 '+parsed.length+' \u9879'}if(typeof parsed==='object'){var keys=Object.keys(parsed);if(!keys.length)return '\u7A7A\u5BF9\u8C61';if(scope==='_sync'&&dataType==='config'&&parsed.enabledToolIds){var ids=parsed.enabledToolIds;var tnames=ids.map(function(id){return toolNameShort(id)});return '\u5DF2\u542F\u7528 '+ids.length+' \u4E2A\u5DE5\u5177\uFF1A'+tnames.slice(0,4).join('\u3001')+(tnames.length>4?'\u2026':'')}if(keys.length<=3){var parts=keys.map(function(k){return k+': '+String(parsed[k]).slice(0,20)});return parts.join(' \u00B7 ')}return keys.length+' \u4E2A\u5B57\u6BB5\uFF1A'+keys.slice(0,3).join('\u3001')+'…'}return String(parsed).slice(0,60)}
function renderFormatted(scope,dataType,parsed){if(parsed===null||parsed===undefined)return '<div class="fmt-empty">\u7A7A</div>';if(typeof parsed==='string')return '<div class="fmt-plain">'+esc(parsed)+'</div>';if(Array.isArray(parsed)){if(!parsed.length)return '<div class="fmt-empty">\u7A7A\u6570\u7EC4</div>';if(scope==='_global'&&dataType==='favorites')return '<div class="fmt-chips">'+parsed.map(function(id){return '<span class="fmt-chip">'+esc(toolNameShort(id))+'</span>'}).join('')+'</div>';if(scope==='_global'&&dataType==='search_history')return '<div class="fmt-chips">'+parsed.map(function(s){return '<span class="fmt-chip">'+esc(s)+'</span>'}).join('')+'</div>';if(scope==='_global'&&dataType==='records')return renderRecords(parsed);if(parsed.length>50){var shown=parsed.slice(0,50);var rest=parsed.length-50;return '<div class="fmt-cards">'+shown.map(function(item,i){return renderValue(item,'\u7B2C'+(i+1)+'\u6761')}).join('')+'</div><div class="fmt-note">\u8FD8\u6709 '+rest+' \u6761\u672A\u663E\u793A</div>'}return '<div class="fmt-cards">'+parsed.map(function(item,i){return renderValue(item,'\u7B2C'+(i+1)+'\u6761')}).join('')+'</div>'}if(typeof parsed==='object')return renderObject(parsed);return '<div class="fmt-plain">'+esc(String(parsed))+'</div>'}
function renderValue(val,label){if(val===null||val===undefined)return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-empty">null</div></div>';if(typeof val==='string')return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-plain">'+esc(val.length>200?val.slice(0,200)+'\u2026':val)+'</div></div>';if(typeof val==='number'||typeof val==='boolean')return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-plain">'+esc(String(val))+'</div></div>';if(Array.isArray(val)){if(!val.length)return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-empty">\u7A7A</div></div>';if(val.length<=10&&val.every(function(v){return typeof v==='string'}))return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-chips">'+val.map(function(s){return '<span class="fmt-chip">'+esc(s)+'</span>'}).join('')+'</div></div>';return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+' (\u5171 '+val.length+' \u9879)</div>'+renderFormatted(null,null,val)+'</div>'}if(typeof val==='object')return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div>'+renderObject(val)+'</div>';return '<div class="fmt-card"><div class="fmt-lbl">'+esc(label)+'</div><div class="fmt-plain">'+esc(String(val))+'</div></div>'}
function renderObject(obj){var keys=Object.keys(obj);if(!keys.length)return '<div class="fmt-empty">\u7A7A\u5BF9\u8C61</div>';var html='';for(var i=0;i<keys.length;i++){var k=keys[i];var v=obj[k];var lbl=k;if(k==='toolId'&&typeof v==='string'){var tn=toolName(v);lbl=tn?'toolId: '+tn:'toolId'}else if(k==='enabledToolIds'&&Array.isArray(v)){html+='<div class="fmt-section"><div class="fmt-lbl">'+esc(k)+'</div><div class="fmt-chips">'+v.map(function(id){return '<span class="fmt-chip">'+esc(toolNameShort(id))+'</span>'}).join('')+'</div></div>';continue}else if(k==='favorites'&&Array.isArray(v)){html+='<div class="fmt-section"><div class="fmt-lbl">\u6536\u85CF\u5DE5\u5177</div><div class="fmt-chips">'+v.map(function(id){return '<span class="fmt-chip">'+esc(toolNameShort(id))+'</span>'}).join('')+'</div></div>';continue}if(v!==null&&v!==undefined&&typeof v==='object'){html+='<div class="fmt-section"><div class="fmt-lbl">'+esc(lbl)+'</div>'+renderValue(v,'')+'</div>'}else{html+='<dl class="fmt-kv"><dt>'+esc(lbl)+'</dt><dd>'+(v===null||v===undefined?'\u7A7A':esc(String(v).slice(0,200)))+'</dd></dl>'}}return html}
function renderRecords(arr){var html='<div class="fmt-cards">';var show=arr.length>20?arr.slice(0,20):arr;for(var i=0;i<show.length;i++){var r=show[i];var name=r.toolName||(r.toolId?toolNameShort(r.toolId):'\u672A\u77E5');var time=r.timestamp?fmtTime(r.timestamp):'';html+='<div class="fmt-card" style="display:flex;justify-content:space-between;align-items:center"><span class="fmt-tool-name">'+esc(name)+'</span><span style="font-size:12px;color:var(--text-muted)">'+esc(time)+'</span></div>'}html+='</div>';if(arr.length>20)html+='<div class="fmt-note">\u8FD8\u6709 '+(arr.length-20)+' \u6761\u672A\u663E\u793A</div>';return html}
